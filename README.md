...
**`main.py`** — the entry point: the welcome banner and the role selection menu that loads the right module.

**`password.py`** — the login module. It checks the username case-insensitively, verifies the password, offers a password reset when it doesn't match, and blocks anyone who isn't on the list. The other three role modules reuse this same login gate.

**`Administrator.py`** — the full admin module:

- **Approve / reject proposals** — reads pending events, asks for a decision on each one, writes approved events to `Approved_events.txt`
- **System statistics** — total approved events, total registered participants, and the most popular event (calculated by counting registrations per event)
- **Report generation** — writes events, participants and organizer reports out to their own `.txt` files
- **User role management** — promotes a participant to organizer, or demotes an organizer back to participant, by moving their name between files
- Every file operation is wrapped in error handling for missing files, permission errors and unexpected failures

See a real run of the module in [`docs/sample-session.md`](docs/sample-session.md).
